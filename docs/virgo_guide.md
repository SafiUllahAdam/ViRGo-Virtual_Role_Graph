...
**Fixed settings:** `seed = 42`; embedding `dimensions = 64, num_walks = 10, window = 10, epochs = 1`; `walk_length` = **80** everywhere (train.py, benchmark_config, notebook) — the paper value (≈1.87× slower than 40, see `docs/notes.md`). Classifier = logistic regression (the paper's L-BFGS / L2). No separate validation set (we don't tune).
