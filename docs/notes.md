...
- [ ] Provide `labels/cora.labels`, `labels/citeseer.labels` — node-classification blocker. `make_planetoid_labels.py` (Planetoid download) is the path, but the download **timed out inside the sandbox** (`FSTimeoutError`); it should work on the real machine — run Step 3 of the notebook.
...
- [ ] Build cached I2V variant; verify embeddings identical + measure speedup (Deliverable #1).
- [ ] Pin walk-length (40 vs 80) across all runs.
